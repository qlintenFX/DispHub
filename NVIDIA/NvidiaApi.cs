...
/// Replaces vibranceDLL.dll with native NVAPI calls for full DVC range support.
/// nvapi.dll only exports nvapi_QueryInterface by name; all other functions
/// are obtained via their well-known IDs (stable across driver versions).
/// </summary>
internal static class NvidiaApi
{
    ...
    // NVAPI Function IDs (extended - supports desaturation/under-saturated levels)
    ...
    private delegate int NvAPI_EnumPhysicalGPUs_t(
        [In, Out] IntPtr[] gpuHandles,
        out int gpuCount);

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate int NvAPI_GPU_GetSystemType_t(
        IntPtr gpuHandle,
        out int systemType);

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate int NvAPI_EnumNvidiaDisplayHandle_t(
        int thisEnum,
        out IntPtr displayHandle);

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate int NvAPI_GetDVCInfo_t(
        IntPtr displayHandle,
        uint outputId,
        ref NvDisplayDvcInfo info);

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate int NvAPI_SetDVCLevel_t(
        IntPtr displayHandle,
        uint outputId,
        int level);

    // Extended delegates (for under-saturated/desaturation support)
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate int NvAPI_GetDVCInfoEx_t(
        IntPtr displayHandle,
        uint outputId,
        ref NvDisplayDvcInfoEx info);

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate int NvAPI_SetDVCLevelEx_t(
        IntPtr displayHandle,
        uint outputId,
        ref NvDisplayDvcInfoEx info);

    // Cached function pointers (standard)
    ...
    // Cached function pointers (extended)
    ...
    /// <summary>
    /// Whether the extended DVC functions (desaturation support) are available.
    /// </summary>
    public static bool HasExtendedDvc => _getDVCInfoEx != null && _setDVCLevelEx != null;

    /// <summary>
    /// The only function directly exported by nvapi.dll.
    /// Returns a function pointer for the given NVAPI function ID.
    /// </summary>
    ...
    /// <summary>
    /// Loads NVAPI function pointers via QueryInterface. Must be called before any other method.
    /// </summary>
    public static bool Load()
    {
        ...
        try
        {
            ...
            // Extended functions (optional - for desaturation support)
            ...
            if (_loaded && HasExtendedDvc)
            {
                Logger.Log("NVAPI loaded with extended DVC support (desaturation available)");
            }
            else if (_loaded)
            {
                Logger.Log("NVAPI loaded (standard DVC only, no desaturation)");
            }
    ...
    // --- Standard DVC functions (0 to max, no desaturation) ---
    ...
    // --- Extended DVC functions (full range including desaturation) ---
    ...
    public static bool GetDVCInfoEx(IntPtr displayHandle, out NvDisplayDvcInfoEx info)
    {
        ...
        // Version: sizeof(struct) | (version << 16)
        info.Version = (uint)Marshal.SizeOf<NvDisplayDvcInfoEx>() | 0x10000;
        return _getDVCInfoEx(displayHandle, 0, ref info) == NVAPI_OK;
    }

    /// <summary>
    /// Sets DVC level using the extended API. The level is passed via the struct's CurrentLevel field.
    /// </summary>
